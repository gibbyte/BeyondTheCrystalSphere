...
void win32_handle_key_up(Input_Key_Code code, s64 gamepad_index)
{
	...
	if (last_state & INPUT_STATE_DOWN)
		state |= INPUT_STATE_JUST_RELEASED;
...
LRESULT CALLBACK win32_window_proc(HWND passed_window, UINT message, WPARAM wparam, LPARAM lparam)
{
	...
	case WM_KEYDOWN:
		...
		if (is_repeat)
			win32_handle_key_repeat(os_key_to_key_code((void *)wparam), -1);
		else
			win32_handle_key_down(os_key_to_key_code((void *)wparam), -1);
		// goto DEFAULT_HANDLE;
		break;
	case WM_SYSKEYUP:
	case WM_KEYUP:
...
void win32_audio_poll_default_device_thread(Thread *t);
...
void win32_query_monitors()
{
	...
	if (os.monitors)
		growing_array_clear((void **)&os.monitors);
	else
		growing_array_init((void **)&os.monitors, sizeof(Os_Monitor), get_heap_allocator());
...
void os_write_string_to_stdout(string s)
{
	...
	if (win32_stdout == INVALID_HANDLE_VALUE)
		return;
...
u64 os_get_number_of_logical_processors()
{
	...
	if ((u64)program_memory_next > (u64)program_tail)
	{
		...
		for (u64 i = 0; i <= ATTEMPTS; i++)
		{
			if (program_memory_capacity >= new_program_size)
				break; // Another thread might have resized already, causing it to fail here.
...
void win32_audio_init()
{
	...
	WAVEFORMATEXTENSIBLE *format_f32 = (WAVEFORMATEXTENSIBLE *)CoTaskMemAlloc(sizeof(WAVEFORMATEXTENSIBLE));
	...
	format_f32->Format.nBlockAlign = format_f32->Format.nChannels * format_f32->Format.wBitsPerSample / 8;
	format_f32->Format.nAvgBytesPerSec = format_f32->Format.nSamplesPerSec * format_f32->Format.nBlockAlign;
...
void win32_audio_thread(Thread *t)
{
	...
	while (!window.should_close)
		tm_scope("Audio update")
		{
			if (win32_audio_deactivated)
				tm_scope("Retry audio device")
				{
					os_sleep(100);
					mutex_acquire_or_wait(&audio_init_mutex);
					win32_audio_init();
...
void win32_lazy_init_xinput()
{
	if (!win32_xinput)
	{
		...
		if (!win32_xinput)
			win32_xinput = LoadLibraryW(L"xinput1_3.dll");
		if (!win32_xinput)
		{
...
void set_gamepad_vibration(float32 left, float32 right)
{
	...
	if (!XInputGetState)
		XInputGetState = (DWORD(*)(DWORD, XINPUT_STATE *))GetProcAddress(win32_xinput, "XInputGetState");
...
void set_specific_gamepad_vibration(u64 gamepad_index, float32 left, float32 right)
{
	...
	if (!XInputSetState)
		XInputSetState = (DWORD(*)(DWORD, XINPUT_VIBRATION *))GetProcAddress(win32_xinput, "XInputSetState");
	...
	if (r != ERROR_SUCCESS)
	{
		log_warning("Could not set gamepad vibration on gamepad %d", gamepad_index);
	}
}
...
void os_update()
{
	...
	if (window.pixel_width % 2 != 0)
		window.pixel_width += 1;
	if (window.pixel_height % 2 != 0)
		window.pixel_height += 1;
		...
		for (DWORD i = 0; i < XUSER_MAX_COUNT; i++)
		{
			...
			if (r == ERROR_SUCCESS)
			{
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_UP)
					win32_handle_key_down(GAMEPAD_DPAD_UP, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_UP)
					win32_handle_key_up(GAMEPAD_DPAD_UP, i);
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_RIGHT)
					win32_handle_key_down(GAMEPAD_DPAD_RIGHT, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_RIGHT)
					win32_handle_key_up(GAMEPAD_DPAD_RIGHT, i);
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_DOWN)
					win32_handle_key_down(GAMEPAD_DPAD_DOWN, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_DOWN)
					win32_handle_key_up(GAMEPAD_DPAD_DOWN, i);
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_LEFT)
					win32_handle_key_down(GAMEPAD_DPAD_LEFT, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_DPAD_LEFT)
					win32_handle_key_up(GAMEPAD_DPAD_LEFT, i);
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_START)
					win32_handle_key_down(GAMEPAD_START, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_START)
					win32_handle_key_up(GAMEPAD_START, i);
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_BACK)
					win32_handle_key_down(GAMEPAD_BACK, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_BACK)
					win32_handle_key_up(GAMEPAD_BACK, i);
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_THUMB)
					win32_handle_key_down(GAMEPAD_LEFT_STICK, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_THUMB)
					win32_handle_key_up(GAMEPAD_LEFT_STICK, i);
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_THUMB)
					win32_handle_key_down(GAMEPAD_RIGHT_STICK, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_THUMB)
					win32_handle_key_up(GAMEPAD_RIGHT_STICK, i);
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER)
					win32_handle_key_down(GAMEPAD_LEFT_BUMPER, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_LEFT_SHOULDER)
					win32_handle_key_up(GAMEPAD_LEFT_BUMPER, i);
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER)
					win32_handle_key_down(GAMEPAD_RIGHT_BUMPER, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_RIGHT_SHOULDER)
					win32_handle_key_up(GAMEPAD_RIGHT_BUMPER, i);
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_A)
					win32_handle_key_down(GAMEPAD_A, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_A)
					win32_handle_key_up(GAMEPAD_A, i);
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_B)
					win32_handle_key_down(GAMEPAD_B, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_B)
					win32_handle_key_up(GAMEPAD_B, i);
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_X)
					win32_handle_key_down(GAMEPAD_X, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_X)
					win32_handle_key_up(GAMEPAD_X, i);
				if (state.Gamepad.wButtons & XINPUT_GAMEPAD_Y)
					win32_handle_key_down(GAMEPAD_Y, i);
				else if (last_states[i].Gamepad.wButtons & XINPUT_GAMEPAD_Y)
					win32_handle_key_up(GAMEPAD_Y, i);
				...
				if (state.Gamepad.bLeftTrigger >= 230)
					win32_handle_key_down(GAMEPAD_LEFT_TRIGGER, i);
				else if (last_states[i].Gamepad.bLeftTrigger >= 230)
					win32_handle_key_up(GAMEPAD_LEFT_TRIGGER, i);
				if (state.Gamepad.bRightTrigger >= 230)
					win32_handle_key_down(GAMEPAD_RIGHT_TRIGGER, i);
				else if (last_states[i].Gamepad.bRightTrigger >= 230)
					win32_handle_key_up(GAMEPAD_RIGHT_TRIGGER, i);

				if (fabsf(input_frame.left_stick.x) < deadzone_left_stick.x)
					input_frame.left_stick.x = 0.0;
				if (fabsf(input_frame.left_stick.y) < deadzone_left_stick.y)
					input_frame.left_stick.y = 0.0;
				if (fabsf(input_frame.right_stick.x) < deadzone_right_stick.x)
					input_frame.right_stick.x = 0.0;
				if (fabsf(input_frame.right_stick.y) < deadzone_right_stick.y)
					input_frame.right_stick.y = 0.0;
				if (fabsf(input_frame.left_trigger) < deadzone_left_trigger)
					input_frame.left_trigger = 0.0;
				if (fabsf(input_frame.right_trigger) < deadzone_right_trigger)
					input_frame.right_trigger = 0.0;
	...
	// Poll window events
	MSG msg;
	while (input_frame.number_of_events < MAX_EVENTS_PER_FRAME && PeekMessage(&msg, 0, 0, 0, PM_REMOVE))
	{
...
void *key_code_to_os_key(Input_Key_Code key_code)
{
	...
	case KEY_TAB:
		return (void *)VK_TAB;
	case KEY_ENTER:
		return (void *)VK_RETURN;
	case KEY_ESCAPE:
		return (void *)VK_ESCAPE;
	case KEY_SPACEBAR:
		return (void *)VK_SPACE;
	case KEY_DELETE:
		return (void *)VK_DELETE;
	case KEY_ARROW_UP:
		return (void *)VK_UP;
	case KEY_ARROW_DOWN:
		return (void *)VK_DOWN;
	...
	case KEY_ARROW_RIGHT:
		return (void *)VK_RIGHT;
	case KEY_PAGE_UP:
		return (void *)VK_PRIOR;
	case KEY_PAGE_DOWN:
		return (void *)VK_NEXT;
	case KEY_HOME:
		return (void *)VK_HOME;
	case KEY_END:
		return (void *)VK_END;
	case KEY_INSERT:
		return (void *)VK_INSERT;
	case KEY_PAUSE:
		return (void *)VK_PAUSE;
	case KEY_SCROLL_LOCK:
		return (void *)VK_SCROLL;
	case KEY_ALT:
		return (void *)VK_MENU;
	case KEY_CTRL:
		return (void *)VK_CONTROL;
	case KEY_SHIFT:
		return (void *)VK_SHIFT;
	case KEY_CMD:
		return (void *)VK_LWIN; // Assuming left win key for both
	case KEY_F1:
		return (void *)VK_F1;
	case KEY_F2:
		return (void *)VK_F2;
	...
	case KEY_PRINT_SCREEN:
		return (void *)VK_SNAPSHOT;
	case MOUSE_BUTTON_LEFT:
		return (void *)VK_LBUTTON;
	case MOUSE_BUTTON_MIDDLE:
		return (void *)VK_MBUTTON;
	case MOUSE_BUTTON_RIGHT:
		return (void *)VK_RBUTTON;
